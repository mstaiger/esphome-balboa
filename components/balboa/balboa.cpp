...
void Balboa::make_request_() {
  if (!pending_writes_.empty()) {
    ...
    switch (kv->second.type) {
      case BalboaDatapointType::FLOAT:
        this->write_value_(dpp.name, BalboaDatapointType::FLOAT, kv->second.value_float, address);
        break;
      case BalboaDatapointType::ENUM_TEXT:
        this->write_value_(dpp.name, BalboaDatapointType::ENUM_TEXT, kv->second.value_enum, address);
        break;
      case BalboaDatapointType::TEXT:
      case BalboaDatapointType::RAW:
      case BalboaDatapointType::UNSUPPORTED:
...
void Balboa::parse_tx_message_() { this->parse_message_(true); }

void Balboa::parse_rx_message_() { this->parse_message_(false); }

void Balboa::parse_message_(bool is_tx) {
  ...
  if (command == READ_COMMAND) {
    ...
    if (read_req_.dst_adr == src_adr && read_req_.src_adr == dst_adr && read_req_.awaiting_res) {
      if (read_req_.type == 1 && read_req_.obj_names.size() == 1) {
        BalboaDatapointType item_type = BalboaDatapointType(pdata[0] & 0x7F);
        if (item_type == BalboaDatapointType::RAW) {
          std::vector<uint8_t> raw(pdata, pdata + data_len);
          const std::string &datapoint_id = read_req_.obj_names[0];
          this->send_datapoint_(
              BalboaDatapointID{.name = datapoint_id, .address = src_adr},
              BalboaDatapoint{.type = item_type, .value_float = 0, .value_string = "", .value_raw = raw});
        }
        ...
        if (item_num != read_req_.obj_names.size()) {
          ...
          while (tpos < data_len) {
            ...
            uint8_t item_len = pdata[tpos];
            handle_response_(BalboaDatapointID{.name = datapoint_id, .address = src_adr}, pdata + tpos + 1, item_len);
            tpos += item_len + 1;
            item_num++;
          }
        }
    ...
    if (type == 1 && pdata[1] == 1 && data_len >= WRITE_DATA_POS) {
      ...
      switch (BalboaDatapointType(pdata[2])) {
        case BalboaDatapointType::FLOAT:
        case BalboaDatapointType::ENUM_TEXT:
...
void Balboa::handle_response_(const BalboaDatapointID &datapoint_id, const uint8_t *p, uint8_t len) {
  BalboaDatapointType item_type = BalboaDatapointType(p[0] & 0x7F);
  switch (item_type) {
    case BalboaDatapointType::FLOAT: {
      p += 3;
      len -= 3;
      ...
      this->send_datapoint_(
          datapoint_id,
          BalboaDatapoint{.type = item_type, .value_float = item_value, .value_string = "", .value_raw = {}});
      break;
    }
    case BalboaDatapointType::TEXT: {
      p += 3;
      ...
      this->send_datapoint_(datapoint_id,
                            BalboaDatapoint{.type = item_type, .value_float = 0, .value_string = s, .value_raw = {}});
      break;
    }
    case BalboaDatapointType::ENUM_TEXT: {
      p += 3;
      ...
      this->send_datapoint_(
          datapoint_id,
          BalboaDatapoint{.type = item_type, .value_enum = item_value, .value_string = s, .value_raw = {}});
      break;
    }
    case BalboaDatapointType::RAW:
    ...
    case BalboaDatapointType::UNSUPPORTED:
      ...
      this->send_datapoint_(datapoint_id,
                            BalboaDatapoint{.type = item_type, .value_float = 0, .value_string = "", .value_raw = {}});
      break;
  }
}

void Balboa::read_buffer_(int bytes_available) {
...
void Balboa::write_value_(const std::string &object, BalboaDatapointType type, float value, uint32_t address) {
  if (address == 0) {
    address = dst_adr_;
  }
...
void Balboa::request_strings_() {
  ...
  for (iter = objects.begin(); iter != objects.end();) {
    if (request_once_datapoint_ids_.count(BalboaDatapointID{.name = *iter, .address = dst_adr}) == 1 &&
        datapoints_.count(BalboaDatapointID{.name = *iter, .address = dst_adr}) == 1) {
  ...
  // Read Class
  if (objects.size() == 1 && raw_datapoint_ids_.count(BalboaDatapointID{.name = objects[0], .address = dst_adr}) == 1) {
    data.push_back(1);
  } else {
    data.push_back(2);
  }
...
void Balboa::transmit_message_(uint8_t command, const std::vector<uint8_t> &data, uint32_t dst_adr, uint32_t src_adr) {
  if (dst_adr == 0) {
    dst_adr = dst_adr_;
  }
  if (src_adr == 0) {
...
void Balboa::set_float_datapoint_value(const std::string &datapoint_id, float value, uint32_t address) {
  ESP_LOGD(TAG, "Setting datapoint %s to %f", datapoint_id.c_str(), value);
  this->set_datapoint_(
      BalboaDatapointID{.name = datapoint_id, .address = address},
      BalboaDatapoint{.type = BalboaDatapointType::FLOAT, .value_float = value, .value_string = "", .value_raw = {}});
}

void Balboa::set_enum_datapoint_value(const std::string &datapoint_id, uint8_t value, uint32_t address) {
  ESP_LOGD(TAG, "Setting datapoint %s to %u", datapoint_id.c_str(), value);
  this->set_datapoint_(
      BalboaDatapointID{.name = datapoint_id, .address = address},
      BalboaDatapoint{
          .type = BalboaDatapointType::ENUM_TEXT, .value_enum = value, .value_string = "", .value_raw = {}});
}

void Balboa::set_datapoint_(const BalboaDatapointID &datapoint_id, const BalboaDatapoint &value) {
  auto specific = datapoint_id;
  if (specific.address == 0) {
    specific.address = dst_adr_;
  }
  auto any = BalboaDatapointID{.name = datapoint_id.name, .address = 0};
  bool send_specific = true;
  bool send_any = true;
  if (datapoints_.count(specific) == 0) {
    ESP_LOGW(TAG, "Setting unknown datapoint %s", datapoint_id.name.c_str());
  } else {
    BalboaDatapoint old_value = datapoints_[specific];
  ...
  if (datapoints_.count(any) == 0) {
    ...
    BalboaDatapoint old_value = datapoints_[any];
    if (old_value == value) {
      ESP_LOGV(TAG, "Not setting unchanged value for datapoint %s", datapoint_id.name.c_str());
      send_any = false;
    }
...
void Balboa::send_datapoint_(const BalboaDatapointID &datapoint_id, const BalboaDatapoint &value) {
  auto specific = BalboaDatapointID{.name = datapoint_id.name, .address = datapoint_id.address};
  auto any = BalboaDatapointID{.name = datapoint_id.name, .address = 0};
  bool send_specific = true;
  bool send_any = true;
...
void Balboa::register_listener(const std::string &datapoint_id, int8_t request_mod, bool request_once,
  ...
  if (request_mod >= 0 && request_mod < request_datapoint_ids_.size()) {
    ...
    if (request_once) {
      request_once_datapoint_ids_.insert(BalboaDatapointID{.name = datapoint_id, .address = src_adr});
    }
  }
  if (is_raw_datapoint) {
    raw_datapoint_ids_.insert(BalboaDatapointID{.name = datapoint_id, .address = src_adr});
  }
  auto listener = BalboaDatapointListener{
      .datapoint_id = BalboaDatapointID{.name = datapoint_id, .address = src_adr},
      .on_datapoint = func,
  };
...
// Fires a Home Assistant event: "esphome.balboa_event" with the response.
void Balboa::homeassistant_read(const std::string &datapoint_id, uint32_t address) {
  if (address == 0) {
    address = dst_adr_;
  }
  register_listener(datapoint_id, -1, true, [this, datapoint_id](const BalboaDatapoint &datapoint) {
    ...
    capi_.fire_homeassistant_event("esphome.balboa_event", data);
  });
  datapoint_ids_for_read_service_.push(BalboaDatapointID{.name = datapoint_id, .address = address});
}

void Balboa::homeassistant_write(const std::string &datapoint_id, uint8_t value, uint32_t address) {
  set_datapoint_(BalboaDatapointID{.name = datapoint_id, .address = address},
                 BalboaDatapoint{
                     .type = BalboaDatapointType::ENUM_TEXT, .value_enum = value, .value_string = "", .value_raw = {}});
}
