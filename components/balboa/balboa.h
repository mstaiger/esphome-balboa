...
struct BalboaDatapointID {
  std::string name;
  uint32_t address;
};
inline bool operator==(const BalboaDatapointID &lhs, const BalboaDatapointID &rhs) {
...
enum class BalboaDatapointType : uint8_t {
  FLOAT = 0,
  TEXT = 1,
  ENUM_TEXT = 2,
  RAW = 4,
...
struct BalboaDatapoint {
  BalboaDatapointType type;
  union {
    float value_float;
    uint8_t value_enum;
  };
...
inline bool operator==(const BalboaDatapoint &lhs, const BalboaDatapoint &rhs) {
...
struct BalboaDatapointListener {
  BalboaDatapointID datapoint_id;
  std::function<void(BalboaDatapoint)> on_datapoint;
};
...
class Balboa : public Component, public uart::UARTDevice {
 public:
  ...
  void register_listener(const std::string &datapoint_id, int8_t request_mod, bool request_once,
                         const std::function<void(BalboaDatapoint)> &func, bool is_raw_datapoint = false,
                         uint32_t src_adr = 0);
 ...
 protected:
  ...
  std::vector<BalboaDatapointListener> listeners_;
  ReadRequest read_req_;
  void set_datapoint_(const BalboaDatapointID &datapoint_id, const BalboaDatapoint &value);
  void send_datapoint_(const BalboaDatapointID &datapoint_id, const BalboaDatapoint &value);
  ...
  void handle_response_(const BalboaDatapointID &datapoint_id, const uint8_t *p, uint8_t len);
  ...
  void write_value_(const std::string &object, BalboaDatapointType type, float value, uint32_t address = 0);
  ...
  std::set<BalboaDatapointID> raw_datapoint_ids_;
  std::set<BalboaDatapointID> request_once_datapoint_ids_;
  std::map<BalboaDatapointID, BalboaDatapoint> datapoints_;
  std::map<BalboaDatapointID, BalboaDatapoint> pending_writes_;
  std::queue<BalboaDatapointID> datapoint_ids_for_read_service_;
